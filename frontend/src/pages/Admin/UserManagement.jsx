...
export default function ItineraryManagement() {
    ...
    const fetchItineraries = async () => {
        ...
        if (result.success) {
            ...
            setTotalPages(0);
            setItineraryError(
                result.message || "Unable to load itineraries. Please try again.",
            );
        }
    ...
    const fetchTypes = async () => {
        ...
        if (result.success) {
            ...
            console.error(result.message);
            setTypesError(
                result.message || "Unable to load itinerary types. Please try again.",
            );
        }
    };
    ...
    return (
        <div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden mb-5">
                ...
                {!loadingItineraries && itineraryError && (
                    <div className="py-16 text-center">
                        ...
                        <p className="text-sm text-gray-400 mb-4">{itineraryError}</p>

                        <button
                            onClick={fetchItineraries}
                            className="text-sm font-medium text-gray-900 hover:underline underline-offset-2 cursor-pointer">
                            Try again
                        </button>
                    </div>
                )}
            ...
            {/* Pagination */}
            {!loadingItineraries && !itineraryError && totalPages > 0 && (
                <div className="flex items-center justify-center gap-3 mb-12">
                    ...
                    <span className="text-sm text-gray-500">
                        Page{" "}
                        <span className="font-semibold text-gray-900">{page + 1}</span> of{" "}
                        {totalPages}
                    </span>
            ...
            <div className="border-t border-gray-100 pt-10">
                ...
                {typesError && (
                    <div className="mb-5">
                        <p className="text-sm text-gray-500 mb-2">{typesError}</p>
